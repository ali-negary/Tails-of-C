...
void gauss(double a[][MAX + 1], double x[MAX], const int n) {
    ...
    for (i = 1; i < n; i++) {
        ...
        int partition_size = (n - i) / threads; // 3 ==> 4,4,4,4,4,4,4,3
        int remainder = (n - i) % threads;      // 7 ==> 6,5,4,3,2,1,0
        int threadIndex;
        int result;
        int new_partition_size;
        // temp <= (n-i) * (n-i) / threshold    ===> temp between 1 & threads
        if (partition_size >= 4 && threads > 1) {
            
            pivotRoutineArguments thread_arguments[threads];
            //start thread
//            printf("++++++++++++++++++++++++++++++++++++++++++++++\n");
            int lowerBound = i + 1;
...
//    printf("i = %d, Rows %d and %d, Psize= %d, THindex = %d\n", i, lowerRowLimit, upperRowLimit,upperRowLimit - lowerRowLimit + 1, p->threadIndex);
